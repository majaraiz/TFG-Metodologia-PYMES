...
class GeneradorConfiguraciones:
    ...
    def generar_configuracion(self, tipo_dispositivo, solo_cifrado=False, addon_sede_central=False):
        """Genera la configuracion final usando plantilla Jinja2"""
        try:
        # Determinar plantilla a usar
            if solo_cifrado:
                plantilla_nombre = "cifrado_addon.j2"
            elif addon_sede_central:
                plantilla_nombre = "cifrado_sede_central_addon.j2"
...
def main():
    ...
    parser.add_argument('--solo-cifrado', action='store_true',
                   help='Generar solo configuracion de cifrado incremental')
    parser.add_argument('--addon-sede-central-cifrado', action='store_true',
                   help='Generar configuracion para añadir esta sede al cifrado de sede central')
    ...
    if args.addon_sede_central_cifrado:
            if not generador.sede_config['sede']['bloques_activos']['cifrado']:
                print("[ERROR] La sede no tiene cifrado activo. No se puede generar addon para sede central")
                sys.exit(1)
            print("[INFO] Generando addon de cifrado para sede central...")
    ...
    configuracion = generador.generar_configuracion(args.dispositivo, args.solo_cifrado, args.addon_sede_central_cifrado)
